...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  Stream<LoginState> mapEventToState(LoginState currentState, LoginEvent event) async* {
    if (event is LoginButtonPressed) {
      ...
      try {
        final User user = await userRepository.authenticate(username: event.username, password: event.password);
        ...
        try {
          if (error.code == "ERROR_USER_NOT_FOUND") {
            final User user = await userRepository.createUser(username: event.username, password: event.password);
            authenticationBloc.dispatch(LoggedIn(user: user));
            yield LoginInitial();
          } else {
            throw error;
